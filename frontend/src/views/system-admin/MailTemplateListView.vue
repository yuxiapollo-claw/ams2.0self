  ...
  <section class="workbench">
    <PageHero :kicker="copy.kicker" :title="copy.title" :subtitle="copy.subtitle">
      <template #actions>
        <button
          class="hero-action"
          data-testid="create-mail-template"
          type="button"
    ...
    <template v-else>
      <FilterPanel :title="copy.filters.title">
        <div class="toolbar">
          <label class="toolbar__field" for="mail-template-keyword">
          ...
          <label class="toolbar__field" for="mail-template-status">
            <span>{{ copy.filters.status }}</span>
            <select id="mail-template-status" v-model="statusFilter" class="toolbar__control" name="statusFilter">
      ...
      <section class="table-card">
        <header class="table-card__header">
          ...
          <button
            class="inline-action"
            data-testid="refresh-mail-templates"
            type="button"
          ...
          <table class="result-table">
            ...
            <tbody>
              <tr v-for="item in filteredItems" :key="item.id">
                <td>
                  <div class="primary-cell">{{ item.templateName }}</div>
                </td>
                <td>
                  <div class="secondary-cell">{{ item.description || copy.descriptionEmpty }}</div>
                </td>
                <td>
                  <div class="secondary-cell">{{ item.subject }}</div>
                </td>
                <td>
                  <div class="secondary-cell">{{ item.body }}</div>
                </td>
                <td>
                  <span
                    class="status-pill"
      ...
      <aside class="drawer-shell__panel" :aria-label="drawerMode === 'create' ? copy.drawer.createTitle : copy.drawer.editTitle">
        <form class="drawer-card" @submit.prevent="handleDrawerSubmit">
          ...
          <div class="drawer-card__body">
            <label class="form-field" for="template-name">
              <span>{{ copy.form.templateName }}</span>
              <input id="template-name" v-model="form.templateName" class="toolbar__control" name="templateName" type="text" />
            </label>

            <label class="form-field" for="template-description">
            ...
            <label class="form-field" for="template-subject">
              <span>{{ copy.form.subject }}</span>
              <input id="template-subject" v-model="form.subject" class="toolbar__control" name="subject" type="text" />
            </label>

            <label class="form-field" for="template-body">
              <span>{{ copy.form.body }}</span>
              <textarea
                id="template-body"
                v-model="form.body"
                class="toolbar__control toolbar__control--textarea toolbar__control--body"
                name="body"
              />
            </label>

            <label class="form-field" for="template-status">
              <span>{{ copy.form.status }}</span>
              <select id="template-status" v-model="form.status" class="toolbar__control" name="status">
                <option value="ENABLED">{{ copy.status.enabled }}</option>
                <option value="DISABLED">{{ copy.status.disabled }}</option>
              </select>
            </label>
...
import {
  ...
  type MailTemplateMutationPayload
...
const zhCopy: Copy = {
  kicker: '系统管理',
  title: '邮件模板配置',
  subtitle: '维护流程通知标题、正文和启停状态。',
  empty: '当前筛选条件下没有匹配的邮件模板。',
  descriptionEmpty: '暂无说明',
  filters: {
    title: '筛选条件',
    search: '关键字',
    searchPlaceholder: '搜索模板名称、主题或说明',
    status: '状态',
    allStatuses: '全部状态'
  },
  table: {
  ...
  error: {
    loadTitle: '加载邮件模板失败',
    actionTitle: '操作失败',
    initialFallback: '邮件模板加载失败，请稍后重试。',
    refreshFallback: '刷新邮件模板失败，请稍后重试。',
    saveFallback: '保存邮件模板失败，请检查输入后重试。',
    deleteFallback: '删除邮件模板失败，请稍后重试。'
  },
  confirmDelete: '确认删除该邮件模板吗？',
  status: {
    enabled: '启用',
    disabled: '停用'
  },
...
const enCopy: Copy = {
  ...
  subtitle: 'Maintain workflow notification subjects, bodies, and lifecycle status.',
  empty: 'No mail template matches the current filters.',
  descriptionEmpty: 'No description',
  filters: {
    title: 'Filters',
    search: 'Keyword',
    searchPlaceholder: 'Search template name, subject, or description',
    status: 'Status',
  ...
  error: {
    loadTitle: 'Failed to load mail templates',
    actionTitle: 'Action failed',
    initialFallback: 'Failed to load mail templates. Try again later.',
    refreshFallback: 'Failed to refresh mail templates. Try again later.',
    saveFallback: 'Failed to save the mail template. Check the form and retry.',
    deleteFallback: 'Failed to delete the mail template. Try again later.'
  },
  confirmDelete: 'Delete this mail template?',
  status: {
    enabled: 'Enabled',
    disabled: 'Disabled'
  },
  metrics: {
    ...
    enabled: 'Enabled',
    disabled: 'Disabled',
    longBody: 'Long Bodies'
  },
  loading: {
    initial: 'Loading mail templates...',
    retry: 'Reloading mail templates...'
  }
}

const { locale } = useI18nText()

const items = ref<MailTemplateItem[]>([])
...
const selectedItem = ref<MailTemplateItem | null>(null)
const drawerSubmitting = ref(false)
const drawerErrorMessage = ref('')
const activeRowId = ref('')
const refreshLoading = ref(false)
const pageLoadingMessage = ref('')
const form = ref<MailTemplateMutationPayload>(emptyForm())
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    const matchesKeyword =
      ...
      [item.templateName, item.subject, item.description, item.body]
...
const metrics = computed(() => [
  ...
  { label: copy.value.metrics.longBody, value: items.value.filter((item) => item.body.trim().length >= 20).length }
])
...
async function loadPage(force = false) {
  ...
  try {
    items.value = await fetchMailTemplates()
  } catch (error) {
    items.value = []
...
function openEditDrawer(item: MailTemplateItem) {
  if (interactionLocked.value) {
    return
  }
  ...
  try {
    const savedItem =
      drawerMode.value === 'create'
        ...
        : selectedItem.value
          ? await updateMailTemplate(selectedItem.value.id, payload)
          : null
...
async function removeItem(item: MailTemplateItem) {
  if (interactionLocked.value) {
    return
  }
  ...
  try {
    await deleteMailTemplate(item.id)
...
function normalizePayload(payload: MailTemplateMutationPayload): MailTemplateMutationPayload {
  return {
...
function upsertItem(nextItem: MailTemplateItem) {
...
function emptyForm(): MailTemplateMutationPayload {
  return {
    templateName: '',
    description: '',
    subject: '',
